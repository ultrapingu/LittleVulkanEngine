...
namespace lve
{
	...
	std::vector<VkVertexInputAttributeDescription> Model::Vertex::getAttributeDescriptions()
	{
		...
		attributeDescriptions.push_back({ 0, 0, VK_FORMAT_R32G32B32_SFLOAT, 0 });							// Position
		...
		const std::string& filepath)
	{
		...
		if (!tinyobj::LoadObj(&attrib, &shapes, &material, &warn, &err, enginePath.c_str()))
	...
	Model::Model(
		Device& device, 
		const Builder& builder) : device{device}
	{
